...
export default function ShapelerEyes({ pupil="Normal", type, baby=false, male=true, blinkDurationCenter=200, blinkDurationSpread=100, blinkDelayCenter=5000, blinkDelaySpread=1000, editing=false }: { pupil?: "Normal" | "Blink" | "Crazy" | "Dizzy" | "Excited" | "Small", type: "Bloopler" | "Googler" | "Sprickler" | "Stronkler", baby?:boolean, male?:boolean, blinkDurationCenter?:number, blinkDurationSpread?:number, blinkDelayCenter?:number, blinkDelaySpread?:number, editing?:boolean }) {
    ...
    return(
        <div className="flex flex-row justify-center items-center space-between w-full h-full absolute">
            ...
            <img className={`relative ${!baby && pupil == "Dizzy" ? "animate-spin" : ""}`} style={{bottom: selectedOffsets.pupils.y + (editing ? pupilLookOffset.y : 0), right: selectedOffsets.pupils.x + (editing ? pupilLookOffset.x : 0)}} src={baby ? `./Parts/Shapelet/Eyes/Pupils/Shapelet-Pupil-${(editing && mouseDown) || isBlinking ? "Blink" : (pupil == "Normal" || pupil == "Blink" ? pupil : "Normal")}.png` : `./Parts/Shapeler/Eyes/Pupils/Pupil-${(editing && mouseDown) || isBlinking ? ((pupil == "Excited" || pupil=="Dizzy") ? pupil : "Blink") : pupil}.png` } />
            <img className={`relative ${!baby && pupil == "Dizzy" ? "animate-spin" : ""}`} style={{bottom: selectedOffsets.pupils.y + (editing ? pupilLookOffset.y : 0), left: selectedOffsets.pupils.x - (editing ? pupilLookOffset.x : 0)}} src={baby ? `./Parts/Shapelet/Eyes/Pupils/Right/Shapelet-Pupil-${(editing && mouseDown) || isBlinking ? "Blink" : (pupil == "Normal" || pupil == "Blink" ? pupil : "Normal")}.png` : `./Parts/Shapeler/Eyes/Pupils/Right/Pupil-${(editing && mouseDown) || isBlinking ? ((pupil == "Excited" || pupil=="Dizzy") ? pupil : "Blink") : pupil}.png` } />
        </div>
    )
}